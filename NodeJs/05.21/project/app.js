...
// const file = require(`./localHost/Data.json`)
...
const server = http.createServer((req, res)=>{
  ...
  if(req.method === `GET`){
    ...
    if(req.url.startsWith(`/?`)){
      ...
      if(resultData){
        const jsonData = {
          'nameInputResult': `${resultData}` 
        }
        const jsonDataString = JSON.stringify(jsonData, null, 2)
        fs.writeFileSync(`./localHost/Data.json`, jsonDataString);
        fetch(`./localHost/Data.json`)
        .then((res)=>{
          console.log(`데이터 파일이 들여와졌습니다.`)
          res.end()
        })
        res.end();
      }else{
